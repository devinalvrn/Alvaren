...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("Alvaren").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("Alvaren").info(f"Invalid Git Command")
        ...
        LOGGER("Alvaren").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("Alvaren").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk Alvaren-UserBot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @rensixy ✨"
    try:
