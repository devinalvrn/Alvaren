...
from Alvaren.modules.help import add_command_help
from Alvaren.utils.misc import extract_user, extract_user_and_reason, list_admins
...
async def member_ban(client: Client, message: Message):
    ...
    msg = (
        f"**Membunuh anak anjing:** {mention}\n"
        f"**Dibunuh Oleh:** {message.from_user.mention if message.from_user else 'Anon'}\n"
    )
...
async def kick_user(client: Client, message: Message):
    ...
    mention = (await client.get_users(user_id)).mention
    msg = f"""
**Mengsleding anak babi:** {mention}
**Disleding Oleh:** {message.from_user.mention if message.from_user else 'Anon'}"""
    if message.command[0][0] == "d":
        await message.reply_to_message.delete()
    if reason:
        msg += f"\n**Reason:** `{reason}`"
