...
from Alvaren import BOTLOG_CHATID
from Alvaren.helpers.msg_types import Types, get_message_type
from Alvaren.helpers.parser import escape_markdown, mention_markdown
from Alvaren.helpers.SQL.afk_db import get_afk, set_afk
...
async def no_longer_afk(client: Client, message: Message):
    ...
    if get and get["afk"]:
        set_afk(False, "")
        try:
            await client.send_message(BOTLOG_CHATID, "Anda sudah tidak lagi Mati!")
        except BaseException:
            pass
        text = "<b>Total {} Mention Saat Sedang Mati<b>\n".format(len(MENTIONED))
        for x in MENTIONED:
            msg_text = x["text"]
            if len(msg_text) >= 11:
