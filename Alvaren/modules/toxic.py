...
from Alvaren.helpers.PyroHelpers import ReplyCheck
from Alvaren.utils import extract_user
...
async def ngejamet(client: Client, message: Message):
    ...
    await asyncio.sleep(1.5)
    await xx.edit("**BOCAH KAMPUNG BABU NGUYUNG**")
    await asyncio.sleep(1.5)
    await xx.edit("**MENTAL TEMPE KAYA LONTE**")
    await asyncio.sleep(1.5)
    await xx.edit("**LEMBEK NGENTOT, NGUYUNG NI BOS🔥**")
...
async def toxicpp(client: Client, message: Message):
    ...
    await asyncio.gather(
        ...
        client.send_message(
            message.chat.id,
            "PASANG PP DULU GOBLOK,BIAR ORANG-ORANG PADA TAU BETAPA HINA NYA MUKA LU KALAU DIKENCINGIN WARGA NGUYUNG😆",
            reply_to_message_id=ReplyCheck(message),
        ),
    )
...
async def toxicdp(client: Client, message: Message):
    ...
    await asyncio.gather(
        ...
        client.send_message(
            message.chat.id,
            "MUKA LU HINA, GAUSAH SOK KERAS YA ANJENG, CUKUP NGUYUNG AJA YANG KERAS!!",
            reply_to_message_id=ReplyCheck(message),
        ),
    )
...
async def toxicvirtual(client: Client, message: Message):
    ...
    await asyncio.sleep(1.5)
    await xx.edit("**KASIAN MANA MASIH MUDA MENDING JOIN NGUYUNG**")
